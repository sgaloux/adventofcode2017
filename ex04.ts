...
function validPhrase(input: string): boolean {
  const words = input.split(" ");
  for (let i in words) {
    const count = countInArray(words, words[i]);
    if (count > 1) {
      return false;
    }
  }
...
const ok = validPhrase("aa bb cc dd aaa");
ok;

const phrasesRaw = `oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
const raw = phrases.length;
raw;
const validPhrasesLenght = phrases.filter(validPhrase).length;
validPhrasesLenght;
